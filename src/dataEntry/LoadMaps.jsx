...
const LoadMaps = () => {
    ...
    const loadedMaps = useLoaderData();
        ...
        fetch(`https://polling-station-management-server.vercel.app/map/${_id}`, {
            ...
            .then((data) => {
                if (data.deletedCount) {
                    ...
                    const remainingData = maps.filter((map) => map._id !== _id);
            ...
            <div className="mt-14 mx-2 my-5 justify-center">
                <div className="flex justify-center justify-items-center">
                    <h1 className="text-xl font-bold text-center mb-10">
                        ম্যাপের সংখ্যা: {toBN(maps.length)}
                    </h1>
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    <Link to="/">
                        <button
                        ...
                        <tbody>
                            {maps.map((map, index) => (
                                ...
                                >
                                    ...
                                    <td>{map.mapLink}</td>
                                    <td>
                                        <Link to={`/dataEntry/map/${map._id}`}>
                                            <button className="btn btn-outline btn-accent m-1">
                                                <HiPencilAlt /> সংশোধন
                                            </button>
                                        </Link>
                                        <button onClick={() => handleDelete(map._id)}
